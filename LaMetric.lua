...
You will need to add one instance of the driver per LaMetric TIME device. Once connected, each driver instacne
will talk to a single device and expose apps installed via resource discovery.
...
- **La metric time host**: IP address or network name of the LaMetric TIME device (see note above).
- **Poll interval**: The number of seconds betwwen polling for resource state updates. Probably
...
the buttins on the device itself, as well as control of some simple settings.

Access to the apps on the device is achieved though separate reources for each app. The supported app resource
types are:
...
allocates addresses to applications. The primary device and all installed apps will be listed
...
The primary use case wold be to send notifications and alerts to be displayed on the TIME device. The device supports
rich customisation of these notifications so we have exposed two commands for this: a basic one to easily send a simple
notification to the device, and an enhanced one giving the full range of customisation.
...
- **\_NOTIFY WITH DETAILS**: Display a customised message on the TIME device. Arguments are populated with defaults
...
You can switch forward and backwards between apps using commands on the TIME device reource. Alternativy you can select
a specific app on the resource for that app by using its *\_ACTIVATE* command.
...
- \_CLOCK APP
    ...
    a new alarm or mofidy an existing alarm if one already exists matching the supplied time.
        - *\_ENABLED*: Turns on the alarm if set to true, turns it off otherwise.
        - *\_TIME*: Local time in format "HH:mm" or "HH:mm:ss".
        - *\_WAKE WITH RADIO*: If true, radio will be activated when alarm goes off.
    - **\_SET CLOCKFACE**: Select the type of icon dispayed when the clock is visible.
        - *\_TYPE*:
            - NONE: No icon is displayed.
            - WEATHER: Displays an icon representing the current weather conditions for your area.
            - PAGE\_A_\DAY: Displays a calander icon or an indicator for special days in your area.
- \_TIMER APP
...
This driver is tested against LaMeteic TIME firmware **v2.3.0**, but should be fully compatible with **v2.1.0**.
